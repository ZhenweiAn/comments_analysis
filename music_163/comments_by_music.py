...
class Comments(object):
    ...
    def get_comments(self, music_id, flag):
        self.headers['Referer'] = 'https://music.163.com/playlist?id=' + str(music_id)
        if flag:
            r = requests.post('https://music.163.com/weapi/v1/resource/comments/R_SO_4_' + str(music_id),
...
if __name__ == '__main__':
    ...
    f = open('w1.txt','w',encoding='utf-8')
    sys.stdout = f
    def save_comments(musics, flag):
        lm = len(musics)
        for i in tqdm(range(lm)[11097:]):
            my_music_id = musics[i]['MUSIC_ID']
            try:
                comments = my_comment.get_comments(my_music_id, flag)
                music_comments = ''
                if comments['total'] > 0:
                    for comment in comments['hotComments']:
                        music_comments += comment['content']
                    sql.insert_comments(my_music_id,music_comments)
